...
public class PreprocessForWord2Vec {
	...
	//String moreDescriptionsFile = "projDesc.csv"; // include descriptions of many projects from Github 
 	Boolean newParseProject = false;
        String moreDescriptionsFile = "C:/Development/NetbeanProjects/data/sim_project/projDesc.csv"; // include descriptions of many projects from Github 
        static String javaProjectPath = "C:/Development/NetbeanProjects/data/sim_project/projects";
	...
	public PreprocessForWord2Vec(){
		...
		wordsDocFreq = new HashMap<>();
		wordsDocFreqByGroup = new ArrayList<>();
		stopWordsByGroup = new ArrayList<>();
		for(int i = 0; i < 4; i++) {
		...
		includeWikiForWord2VecModel = false;//make it true for add wiki article
	...
	public static void main(String[] args) throws IOException{
		...
		preprocess.numOfProjects(javaProjectPath);
		...
		preprocess.parseAndStemProjects(javaProjectPath);
		preprocess.stemProjects(javaProjectPath);
		preprocess.removeProjectsWithNoDescriptionWithNoCheck(javaProjectPath);
		//preprocess.removeProjectsWithNoCodeWithNoCheck(javaProjectPath);
		preprocess.parseAndStemProjectsName(javaProjectPath);
		preprocess.removeTabSpace(javaProjectPath);
		...
		preprocess.wordsStaticsByGroupMore(javaProjectPath);
		//preprocess.generateStopWordsByGroup("english.stop", 0.14);
		preprocess.generateStopWordsByGroupMore("english.stop", 0.35);
		preprocess.removeStopWordsByGroup(javaProjectPath);

		preprocess.generateDocForWord2Vec(javaProjectPath, "forWord2Vec");
		//preprocess.generateDocForWord2VecEvaluation();
		//preprocess.generateDocsForLda(args[0], "forLda");

		preprocess.generateRankedWordsBasedOnTfidfForLdaWmd(javaProjectPath, 25);
		//preprocess.generateRankedWordsBasedOnTfidfForLdaWmdByGroup(javaProjectPath, 25);
